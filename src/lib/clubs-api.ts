...
export async function fetchClubsWithFollowStatus({
  profileId,
  collegeDomain,
}: FetchClubsParams): Promise<ClubProfile[]> {
  assertValidUuid(profileId, "profile id");
  if (!collegeDomain) {
    throw new Error("Missing college domain");
  }

  const { data: clubProfiles, error: clubsError } = await supabase
    .from("profiles")
  ...
  if (clubIds.length > 0) {
    const { data: followerCounts, error: countsError } = await supabase
      .from("connections")
      .select("receiver_id")
...
export async function unfollowClubConnection({
  requesterId,
  clubId,
}: UnfollowClubParams): Promise<void> {
  assertValidUuid(requesterId, "profile id");
  assertValidUuid(clubId, "club id");
