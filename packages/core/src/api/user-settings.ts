...
export async function getUserSettings(
  ...
  const { data: createdLegacy, error: createdLegacyError } = await client
    .from("user_settings")
    .insert({ user_id: userId })
    .select(legacySelect)
    .single();
 ...
 * Uses typeof window guard for cross-platform safety.
 */
...
// NOTE: applyThemeToDOM is web-only and lives in the web adapter layer.
