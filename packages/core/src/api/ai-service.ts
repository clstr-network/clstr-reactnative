/**
 * AI Service — @clstr/core
 *
 * Platform-agnostic AI chat & review functions.
 * Every supabase-using function receives `client: SupabaseClient` as first param.
 *
 * Pure functions (reviewAlumniInviteData, computeDomainSimilarity, simpleHash)
...
export async function createChatSession(client: SupabaseClient): Promise<AIChatSession> {
  ...
  if (error) throw createAppError('Failed to create chat session', 'createChatSession', error);
  return data as AIChatSession;
}

/** Fetch all chat sessions for the current user (most recent first). */
export async function getChatSessions(client: SupabaseClient): Promise<AIChatSession[]> {
  const { data: { session } } = await client.auth.getSession();
  if (!session) return [];
  ...
  if (error) throw createAppError('Failed to get chat sessions', 'getChatSessions', error);
  return (data ?? []) as AIChatSession[];
}
...
export async function getChatMessages(
  ...
  if (error) throw createAppError('Failed to get chat messages', 'getChatMessages', error);
...
export async function saveChatMessage(
  ...
  if (error) throw createAppError('Failed to save chat message', 'saveChatMessage', error);
...
/** Send messages to the AI chat edge function and return the assistant's response. */
export async function sendAIChatMessage(
  ...
  try {
    ...
    throw createAppError(
      'Failed to connect to AI chat. Please try again.',
      'sendAIChatMessage',
      error
    );
  }
...
export async function deleteChatSession(
  ...
  if (error) throw createAppError('Failed to delete chat session', 'deleteChatSession', error);
}
...
export async function updateChatSessionTitle(
  ...
  if (error) throw createAppError('Failed to update chat title', 'updateChatSessionTitle', error);
}

// ─── AI Excel Upload Review (Pure — no Supabase) ────────────

/**
 * Deterministic AI review of validated alumni invite rows.
 * Runs client-side (no LLM call). AI NEVER auto-fixes.
 */
export function reviewAlumniInviteData(input: AIReviewInput): AIReviewOutput {
  ...
  const highRiskRows = new Set(
    dedupedWarnings.filter((w) => w.confidence >= 0.85).map((w) => w.row_index)
  );
...
export function computeDomainSimilarity(a: string, b: string): number {
  if (a === b) return 1;
...
export async function saveAIReviewResult(
  ...
  const { error } = await client.from('ai_review_results').insert({
  ...
  if (error) throw createAppError('Failed to save AI review result', 'saveAIReviewResult', error);
}
