/**
 * Jobs API — shared, platform-agnostic module.
 *
 * Conventions:
 *  • Every Supabase-touching function receives `client` as first arg.
 *  • `handleApiError` → `createAppError`.
 *  • `window.location.origin` → `appUrl` parameter.
 *  • `navigator.share` / `navigator.clipboard` → web-only; shareJob kept
 *    but accepts injected deps so the adapter can supply platform functions.
 */
...
export function isRecommendedJobsRpcUnavailable(err: unknown): boolean {
...
export async function getJobs(
  ...
  try {
    ...
    if (user) {
      ...
      if (!profile?.college_domain) throw new Error("Profile missing college domain");
      userCollegeDomain = profile.college_domain;
    }

    let finalQuery = client
      .from("jobs")
      .select(`*, poster:profiles!jobs_poster_id_fkey(id, full_name, avatar_url, role)`)
      .eq("is_active", true);
    ...
    if (filters.search) {
      ...
      finalQuery = finalQuery.or(
        `job_title.ilike.${searchTerm},company_name.ilike.${searchTerm},description.ilike.${searchTerm}`,
      );
    }
    ...
    if (user && jobs && jobs.length > 0) {
      ...
      const { data: savedJobs } = await client
        .from("saved_items")
        .select("item_id")
        .eq("user_id", user.id)
        .eq("type", "job")
    ...
    const apiError = createAppError("Failed to load jobs. Please try again.", "getJobs", error);
...
export async function getRecommendedJobs(
  ...
  try {
    const { data: { user } } = await client.auth.getUser();
    if (!user) return { jobs: [], error: null };

    const { data, error } = await (client.rpc as any)("get_recommended_jobs_with_scores", {
      p_user_id: user.id,
      p_limit: limit,
    ...
    const apiError = createAppError("Failed to load recommended jobs.", "getRecommendedJobs", error);
    return { jobs: [], error: apiError.message };
  }
}
...
export async function refreshJobMatches(
  ...
  try {
    ...
    const { error } = await (client.rpc as any)("refresh_user_job_matches", {
      p_user_id: user.id,
    });
    ...
    const apiError = createAppError("Failed to refresh job matches.", "refreshJobMatches", error);
    return { success: false, error: apiError.message };
  }
}

export async function getAlumniJobs(
  client: SupabaseClient,
): Promise<{ jobs: Job[]; error: string | null }> {
  try {
    const { data: { user } } = await client.auth.getUser();
    ...
    const { data: alumniProfiles } = await client
      .from("profiles")
      .select("id")
      .eq("role", "Alumni");

    const alumniIds = alumniProfiles?.map((p) => p.id) || [];
    if (alumniIds.length === 0) return { jobs: [], error: null };
    ...
    const apiError = createAppError("Failed to load alumni jobs.", "getAlumniJobs", error);
    return { jobs: [], error: apiError.message };
  }
}
...
export async function getSavedJobs(
  ...
  try {
    ...
    const { data: savedJobs, error: savedError } = await client
      .from("saved_items")
    ...
    if (!savedJobs || savedJobs.length === 0) return { jobs: [], error: null };
    ...
    const apiError = createAppError("Failed to load saved jobs.", "getSavedJobs", error);
    return { jobs: [], error: apiError.message };
  }
}
...
export async function toggleSaveJob(
  ...
  try {
    ...
    if (!job || !job.is_active) throw new Error("Job not found or inactive");

    const matchesDomain = !job.college_domain || job.college_domain === profile.college_domain;
    if (!matchesDomain) throw new Error("Job not accessible for your domain");
    ...
    if (existing) {
      ...
      if (insertError) {
        if (insertError.code === "23505") return { saved: true, error: null };
        throw insertError;
      }
      return { saved: true, error: null };
    }
  } catch (error) {
    const apiError = createAppError("Failed to save/unsave job.", "toggleSaveJob", error);
...
export async function createJob(
  ...
  try {
    ...
    const apiError = createAppError("Failed to create job posting.", "createJob", error);
...
export async function applyToJob(
  ...
  try {
    ...
    const { data: application, error } = await (client as any)
      .from("job_applications")
      .insert({
        job_id: input.job_id,
        user_id: user.id,
    ...
    const apiError = createAppError("Failed to submit application.", "applyToJob", error);
...
export async function getMyApplications(
  ...
  try {
    ...
    const { data: applications, error } = await (client as any)
      .from("job_applications")
      .select(`*, job:jobs(*)`)
      .eq("user_id", user.id)
      .order("created_at", { ascending: false });
    ...
    const apiError = createAppError("Failed to load your applications.", "getMyApplications", error);
...
export async function getJobById(
  ...
  try {
    ...
    if (error) throw error;
    if (!job.is_active) throw new Error("Job not found or inactive");

    if (job.college_domain) {
      if (!userCollegeDomain) throw new Error("Job not accessible without profile domain");
      if (job.college_domain !== userCollegeDomain) throw new Error("Job not accessible for your domain");
    }

    let enrichedJob: Job = job as unknown as Job;
    if (user) {
      const { data: savedJob } = await client
        .from("saved_items")
        .select("id")
        .eq("user_id", user.id)
        .eq("type", "job")
    ...
    const apiError = createAppError("Failed to load job details.", "getJobById", error);
    return { job: null, error: apiError.message };
  }
}

// ============================================================================
// SHARING (platform-injectable)
// ============================================================================
 ...
 */
export function getJobShareUrl(appUrl: string, jobId: string): string {
  return `${appUrl}/jobs/${jobId}`;
}

/**
 * Platform-specific sharing deps (Web Share API / clipboard).
 * The web adapter supplies concrete implementations.
 */
export interface ShareJobDeps {
  /** If available, call the native share sheet; may throw AbortError. */
  nativeShare?: (data: { title: string; text: string; url: string }) => Promise<void>;
...
export async function shareJob(
  ...
  try {
    const shareUrl = getJobShareUrl(deps.appUrl, job.id);
    const shareData = {
    ...
    if (deps.nativeShare) {
      await deps.nativeShare(shareData);
      return { success: true, error: null };
    }

    await deps.copyToClipboard(shareUrl);
    return { success: true, error: null };
  } catch (error) {
