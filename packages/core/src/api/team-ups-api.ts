/**
 * Team-Ups API — Cross-Platform
 *
 * Team matching for hackathons, college events, competitions, and short-term projects.
...
const normalizeCreator = (data: unknown): TeamUpCreator | undefined => {
  if (
    ...
    return {
      ...
      role: d.role ?? 'Student',
      completed_team_ups_count: d.completed_team_ups_count ?? 0,
    };
  }
  return undefined;
};
...
const fetchProfileSummariesById = async (
  ...
  ids: string[],
): Promise<Map<string, TeamUpCreator>> => {
...
export async function getTeamUpRoleDefinitions(
  client: SupabaseClient,
): Promise<{
...
export async function getTeamUps(
  client: SupabaseClient,
  params: GetTeamUpsParams,
): Promise<{
  data: TeamUp[];
  error: string | null;
}> {
  try {
    ...
    const creatorMap = await fetchProfileSummariesById(
      client,
...
export async function getMyTeamUps(
  client: SupabaseClient,
  userId: string,
): Promise<{
  data: TeamUp[];
  ...
  try {
    ...
    const rows = (data ?? []) as TeamUp[];
    const creatorMap = await fetchProfileSummariesById(client, [userId]);
...
export async function createTeamUp(
  client: SupabaseClient,
  params: CreateTeamUpParams,
): Promise<{
  data: TeamUp | null;
  error: string | null;
}> {
  try {
    ...
    const { data: cooldownCheck, error: cooldownError } = await client.rpc(
      'check_team_up_cooldown',
      {
    ...
    if (cooldownError) {
      ...
      throw new Error(
        cooldownCheck.message ||
          'You must wait before creating another team-up for this event',
      );
    }
    ...
    const creatorMap = await fetchProfileSummariesById(client, [params.userId]);
    const teamUp: TeamUp = {
      ...(data as TeamUp),
...
export async function deleteTeamUp(
  client: SupabaseClient,
  teamUpId: string,
  userId: string,
): Promise<{
  ...
  try {
    ...
    const { data: teamUp, error: fetchError } = await client
      .from('team_ups')
...
export async function closeTeamUp(
  client: SupabaseClient,
  teamUpId: string,
  userId: string,
): Promise<{
...
// ============================================================================
// CANCEL TEAM-UP REQUEST
// ============================================================================

export async function cancelTeamUpRequest(
  client: SupabaseClient,
  requestId: string,
  userId: string,
): Promise<{
...
export async function getTeamUpRequests(
  client: SupabaseClient,
  teamUpId: string,
  creatorId: string,
): Promise<{
  ...
  try {
    ...
    const { data: teamUp, error: teamUpError } = await client
      .from('team_ups')
      .select('id, creator_id')
      .eq('id', teamUpId)
      .single();
    ...
    const requesterMap = await fetchProfileSummariesById(
      client,
    ...
    const requests: TeamUpRequest[] = rows.map((row) => ({
      ...row,
      requester:
        requesterMap.get(row.requester_id) ?? fallbackCreator(row.requester_id),
    }));
...
export async function getMyTeamUpRequests(
  client: SupabaseClient,
  userId: string,
): Promise<{
  data: TeamUpRequest[];
  ...
  try {
    ...
    const requests: TeamUpRequest[] = rows.map((row) => ({
      ...row,
      team_up: row.team_up as
        | Pick<TeamUp, 'id' | 'event_name' | 'creator_id'>
        | undefined,
    }));
...
export async function getIncomingTeamUpRequests(
  client: SupabaseClient,
  userId: string,
): Promise<{
  data: TeamUpRequest[];
  ...
  try {
    ...
    return {
      data: [],
      error: getErrorMessage(error, 'Failed to fetch incoming requests'),
    };
  }
}
...
export async function createTeamUpRequest(
  client: SupabaseClient,
  params: CreateTeamUpRequestParams,
): Promise<{
  success: boolean;
  error: string | null;
}> {
  try {
    ...
    const { data: teamUp, error: teamUpError } = await client
      .from('team_ups')
      .select(
        'id, creator_id, status, college_domain, intent, team_size_current, team_size_target',
      )
      .eq('id', params.teamUpId)
      .single();

    if (teamUpError) throw teamUpError;
    if (!teamUp) throw new Error('Team-up not found');
    if (teamUp.status !== 'active') throw new Error('Team-up is no longer active');
    ...
    if (params.requestType === 'join_request') {
      ...
      if (
        teamUp.team_size_target &&
        (teamUp.team_size_current ?? 0) >= teamUp.team_size_target
      ) {
    ...
    const { error } = await client.from('team_up_requests').insert({
...
export async function respondToTeamUpRequest(
  ...
  try {
    ...
    // For accepts, use the safe RPC to prevent race conditions
    if (status === 'accepted') {
      const { data: result, error: rpcError } = await client.rpc(
        'safe_accept_team_up_request',
        {
      ...
      if (rpcError) {
        console.warn(
          'Safe accept RPC failed, falling back to manual handling:',
          rpcError,
        );
      } else if (result) {
        if (!result.success) {
    ...
    return {
      success: false,
      error: getErrorMessage(error, 'Failed to respond to request'),
    };
  }
}
...
export async function getTeamUpMembers(
  client: SupabaseClient,
  teamUpId: string,
): Promise<{
  data: TeamUpMember[];
  ...
  try {
    ...
    const userMap = await fetchProfileSummariesById(
      client,
...
export async function hasUserRequestedTeamUp(
  client: SupabaseClient,
  teamUpId: string,
  userId: string,
): Promise<{
