/**
 * college-utils.ts — shared version
 *
 * Pure functions are direct exports.
 * Server-side checks (isPublicEmailDomainServer) require a SupabaseClient parameter.
 */
 ...
 * Requires a SupabaseClient parameter (no singleton).
 */
export async function isPublicEmailDomainServer(
  ...
  try {
    const { data, error } = await client.rpc("is_public_email_domain", {
      p_domain: normalized,
    });
    if (error) {
