...
export default function OnboardingScreen() {
  ...
  const handleNext = async () => {
    ...
    setIsLoading(true);
    try {
      await signUp({ name: name.trim(), role: role!, department, bio: bio.trim() });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <KeyboardAwareScrollViewCompat
        style={styles.scroll}
        contentContainerStyle={{ paddingTop: insets.top + webTopInset + 40, paddingBottom: insets.bottom + 100, paddingHorizontal: 24 }}
        bottomOffset={20}
      >
        ...
        {step === 0 && (
          <View style={styles.stepContent}>
            ...
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>This will be shown on your profile</Text>
            <TextInput
        ...
        {step === 1 && (
          <View style={styles.stepContent}>
            ...
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>This helps us personalize your experience</Text>
            <View style={styles.roleList}>
              {ROLES.map(r => {
                const badgeColor = getRoleBadgeColor(r.value, colors);
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      styles.roleCard,
                      {
                        backgroundColor: isSelected ? badgeColor + '12' : colors.surface,
                        borderColor: isSelected ? badgeColor : colors.border,
                      },
                    ]}
                  >
                    <View style={[styles.roleIcon, { backgroundColor: badgeColor + '15' }]}>
                      <Ionicons name={r.icon} size={24} color={badgeColor} />
                    </View>
                    ...
                    {isSelected && <Ionicons name="checkmark-circle" size={24} color={badgeColor} />}
                  </Pressable>
                );
              })}
            </View>
        ...
        {step === 2 && (
          <View style={styles.stepContent}>
            ...
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>Select your primary department</Text>
              ...
              {DEPARTMENTS.map(d => (
                <Pressable
                  ...
                  style={[
                    styles.deptChip,
                    {
                      backgroundColor: department === d ? colors.tint : colors.surfaceElevated,
                      borderColor: department === d ? colors.tint : colors.border,
                    },
                  ]}
                >
                  <Text style={[styles.deptText, { color: department === d ? '#fff' : colors.textSecondary }]}>{d}</Text>
                </Pressable>
              ))}
            </View>
          </View>
        ...
        {step === 3 && (
          <View style={styles.stepContent}>
            ...
            <Text style={[styles.stepDesc, { color: colors.textSecondary }]}>Optional - you can always update this later</Text>
            <TextInput
              style={[styles.input, styles.bioInput, { color: colors.text, backgroundColor: colors.surfaceElevated, borderColor: colors.border }]}
      ...
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom + (Platform.OS === 'web' ? 34 : 16), backgroundColor: colors.background }]}>
        <View style={styles.bottomActions}>
          ...
          <Pressable
            ...
            style={({ pressed }) => [
              styles.nextBtn,
              { backgroundColor: canProceed ? colors.tint : colors.surfaceElevated, flex: 1 },
              pressed && { opacity: 0.85 },
            ]}
          >
            <Text style={[styles.nextText, { color: canProceed ? '#fff' : colors.textTertiary }]}>
              {isLoading ? 'Setting up...' : step === 3 ? 'Get Started' : 'Continue'}
            </Text>
